...
ListBox supports deletion of items using the <Keyboard>Backspace</Keyboard> or <Keyboard>Delete</Keyboard> keys. When these keys are pressed, the `onDelete` callback is invoked with a Set of keys representing the items to delete. If items are selected, those keys are passed. Otherwise, if an item is focused, that key is passed.
