...
```tsx
import {ListBox, ListBoxItem, ListBoxSection, Header, Text, SelectionIndicator} from 'react-aria-components';
...
The `onRemove` prop can be used to allow the user to remove items from a ListBox using keyboard shortcuts. When the `onRemove` prop is provided, users can press the <Keyboard>Delete</Keyboard> or <Keyboard>Backspace</Keyboard> keys to remove selected items. If no items are selected, the focused item will be removed. When multiple items are selected, all selected items will be removed at once.
...
The `onRemove` prop can be used to allow the user to remove items from a ListBox using keyboard events. When the <Keyboard>Delete</Keyboard> or <Keyboard>Backspace</Keyboard> key is pressed on a focused item, the `onRemove` callback is called with a Set of keys representing the items to be removed. If the focused item is selected, all selected items will be removed. Otherwise, only the focused item will be removed.
...
When using multiple selection, pressing <Keyboard>Delete</Keyboard> or <Keyboard>Backspace</Keyboard> on a selected item will remove all selected items:
...
Disabled items cannot be removed and will be excluded from the Set of keys passed to `onRemove`.
...
To enable infinite scrolling, render a `<ListBoxLoadMoreItem>` element at the end of the list.
...
<PropTable component={docs.exports.ListBoxLoadMoreItem} links={docs.links} />
