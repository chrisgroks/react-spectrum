...
export const FormValidationExample: DatePickerStory = (args) => {
  const datePickerRef = React.useRef<{focus: () => void}>(null);
...
export const DatePickerWithReactHookForm: DatePickerStory = (args) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!selectedDate) {
      setErrorMessage('Please select a date');
      // Auto-focus the DatePicker when validation fails
      if (datePickerRef.current) {
        datePickerRef.current.focus();
      }
      return;
    ...
    if (selectedJsDate <= today) {
      setErrorMessage('Please select a future date (not today or past dates)');
      // Auto-focus the DatePicker when validation fails
      if (datePickerRef.current) {
        datePickerRef.current.focus();
      }
    ...
    setErrorMessage('');
    action('Form submitted successfully!')(selectedDate);
    alert('Form submitted successfully! Date: ' + selectedJsDate.toDateString());
  };

  return (
    <Form onSubmit={handleSubmit} style={{display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 400}}>
      <h3 style={{margin: 0}}>DatePicker with react-hook-form</h3>
      <p style={{margin: 0, fontSize: '14px', color: '#666'}}>
        This demonstrates ref forwarding for auto-focus on validation errors. 
        Try submitting with no date, a past date, or a future date.
      </p>
      ...
      <DatePicker
        ...
        onChange={setSelectedDate}
        {...args}>
        <Label style={{display: 'block', fontWeight: 'bold'}}>Event Date (must be in the future)</Label>
        <Group style={{display: 'inline-flex'}}>
          <DateInput className={styles.field}>
            {segment => <DateSegment segment={segment} className={clsx(styles.segment, {[styles.placeholder]: segment.isPlaceholder})} />}
          </DateInput>
      ...
      {errorMessage && (
        <div style={{color: 'red', fontSize: '14px', marginTop: -8, padding: '8px', backgroundColor: '#fee', border: '1px solid #fcc', borderRadius: 4}}>
          ⚠️ {errorMessage}
        </div>
      )}
